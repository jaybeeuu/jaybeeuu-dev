...
import { useAsyncGenerator, useIsMounted, usePromise, useSemanticMemo } from "./async-hooks";
...
describe("async-hooks", () => {
  ...
  describe("useSemanticMemo", () => {
    ...
    it("does not rerun the factory when the component rerenders, and the deps haven't updated.", () => {
      const factory = jest.fn<number, [number]>().mockImplementation((memoResult: number) => memoResult);

      const { rerender } = renderHook(
        ({ memoResult }) => useSemanticMemo(
          () => factory(memoResult),
          [ memoResult]
        ),
  ...
  describe("useAsyncGenerator", () => {
    it("returns the initial value before the generator emits.", () => {
      const { result } = renderHook(() => useAsyncGenerator(
    ...
    it("doesn't explode if the component is unmounted before the generator emits.", async () => {
      ...
      const { result, unmount } = renderHook(() => useAsyncGenerator(
        async function* (): AsyncGenerator<number> {
          await promise;
          yield 1;
        },
  ...
  describe("usePromise", () => {
    ...
    it("returns complete after the promise resolves.", async () => {
      ...
      await waitFor(() => {
        expect(result.current.promiseState).toStrictEqual({ status: "complete", value: 1 });
      });
    });

    it("uses the factory to calculate a nw value when the deps updae during a render.", async () => {
      const { result, rerender } = renderHook(({ value }) => usePromise(
      ...
      await waitFor(() => {
        expect(result.current.promiseState).toStrictEqual({ status: "complete", value: 2 });
      });
    });
    ...
    it("aborts the promise if the component unmounts.", async () => {
      ...
      const { unmount } = renderHook(() => usePromise(
        ({ abortSignal }) => {
          const promise = new ControllablePromise();
          abortSignal.onabort = () => {
    ...
    it("aborts the promise if the component rerenders and the deps have updated.", async () => {
      const abortedRef: { current: boolean; } = { current: false };
      const { rerender } = renderHook(({ value }) => usePromise(
        ({ abortSignal }) => {
          const promise = new ControllablePromise();
          abortSignal.onabort = () => {
            abortedRef.current = true;
