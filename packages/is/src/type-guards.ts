...
export const isLiteral = <Type extends string | number | boolean>(
  ...
  isType((candidate: unknown, context): ValidationResult => {
    ...
    return failValidation(
      `Expected literal value "${String(type)}", but received "${typeof candidate}": ${
        typeof candidate !== "object" && typeof candidate !== "undefined"
          ? String(candidate)
          : ""
      }`,
      context
...
> =>
  isType((candidate: unknown, context: ValidationContext): ValidationResult => {
    ...
    return failValidation(
      `Expected "${typeString}", but received "${typeof candidate}"${
        typeof candidate !== "object" && typeof candidate !== "undefined"
          ? `: ${String(candidate)}`
          : ""
      }`,
