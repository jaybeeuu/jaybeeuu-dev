...
> = `${Block}__${Element}`;
export type ModifierIdentifier<
  Block extends string,
...
export interface HookBlock<Prefix extends string, Block extends string> {
  ...
  element: <Element extends string>(
    element: Element,
  ) => ElementIdentifier<ElementIdentifier<Prefix, Block>, Element>;
  childBlock: <NewBlock extends string>(
    block: NewBlock,
  ) => HookBlock<ElementIdentifier<Prefix, Block>, NewBlock>;
  modifier: <Modifier extends string>(
    modifier: Modifier,
  ) => ModifierIdentifier<ElementIdentifier<Prefix, Block>, Modifier>;
  makeModifierFactory: () => <Modifier extends string>(
    modifier: Modifier,
  ) => ModifierIdentifier<ElementIdentifier<Prefix, Block>, Modifier>;
}

const hookBlockFactory =
  <Prefix extends string>(prefix: Prefix) =>
  <Block extends string>(block: Block): HookBlock<Prefix, Block> => {
    const blockClass = `${prefix}__${block}` as const;

    const getBlock: HookBlock<Prefix, Block> = (): ElementIdentifier<
      Prefix,
      Block
    > => {
      return blockClass;
...
export const makeClassSelectors = <Hooks extends HookMap>(
  hooks: Hooks,
): ClassSelectors<Hooks> => {
