...
describe("conv", () => {
  ...
  it("makes an optional variable available if it is present.", () => {
    env.THING = "{thing}";
    const vars = conv({ name: "THING", type: "string", optional: true });
    expect(vars.THING).toBe("{thing}");
  });
  ...
  describe("number", () => {
    it("throws if the variable value is not a number.", () => {
      env.THING = "{thing}";
      expect(() => conv({ name: "THING", type: "number" })).toThrow(
    ...
    it("returns the value converted to a number, if it is a valid number.", () => {
      env.THING = "123.4";
      expect(conv({ name: "THING", type: "number" }).THING).toBe(123.4);
    });
  });
  ...
  describe("boolean", () => {
    it('throws if the variable value is not "true" or "false".', () => {
      env.THING = "{thing}";
      expect(() => conv({ name: "THING", type: "boolean" })).toThrow(
    ...
    it.each([
      ...
      ({ value, expectedResult }) => {
        env.THING = value;
        expect(conv({ name: "THING", type: "boolean" }).THING).toBe(
          expectedResult
        );
      }
    );
  ...
  describe("date", () => {
    it("throws if the variable value is not a valid date.", () => {
      ...
      expect(() => conv({ name: "THING", type: "date" })).toThrow(
    ...
    it.each([
      ...
      ({ value, expectedResult }) => {
        env.THING = value;
        expect(conv({ name: "THING", type: "date" }).THING).toStrictEqual(
          expectedResult
        );
      }
    );
  ...
  describe("types", () => {
    it("is able to resolve the type of the value only from the default property.", () => {
      delete env.THING;
      const vars = conv({ name: "THING", default: 123 });
    ...
    it("is able to resolve the type of the value only from the type property.", () => {
      env.THING = "true";
      const vars = conv({ name: "THING", type: "boolean" });
      const value: boolean = vars.THING;
      expect(value).toBe(true);
    });

    it("defaults the type of the variable to string, when only passed a name.", () => {
      env.THING = "{thing}";
      const vars = conv("THING");
    ...
    it("defaults the type of a variable to string, when passed only optional.", () => {
      env.THING = "{thing}";
